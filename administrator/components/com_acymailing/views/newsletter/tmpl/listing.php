				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center" style="text-align:center" >
						<?php echo acymailing_getDate($row->senddate);
							if(!empty($row->queued) && acymailing_isAllowed($this->config->get('acl_queue_delete','all'))){ ?>
						<br/><button class="btn btn-primary" onclick="if(confirm('<?php echo str_replace("'", "\'", JText::_('ACY_VALID_DELETE_FROM_QUEUE')); ?>')){ window.location.href = '<?php echo JURI::base(); ?>index.php?option=com_acymailing&ctrl=<?php if(!JFactory::getApplication()->isAdmin()) echo 'front'; ?>newsletter&task=cancelNewsletter&<?php echo acymailing_getFormToken(); ?>=1&mailid=<?php echo $row->mailid; ?>'; } return false;"><?php echo JText::_('ACY_CANCEL'); ?></button>
						<?php } ?>
					</td>
					<td align="center" style="text-align:center" >
						<?php
